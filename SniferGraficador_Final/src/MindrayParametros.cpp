...
int MindrayParametros::carfarSubTram(const std::vector<uint8_t> &datas, int pos) {
	...
	if (sub.seleccionMetod(codeType(datas,pos))==0) {
		cout << "pos despues "<<pos<< endl;
		SubTramaECG su(sub.getstart(),sub.getvodi(),sub.getsize());
		 su.clasficaSubTra(datas,pos);
		 su.setSiz(sub.sizeSub());
		 pos=su.addData(datas, pos, su.sizePSubtrama());
		 sub.~SubTramaParam();
		 this->tam+= tama + su.sizeSub();
		 cout << "esto es tam" << tam << "esto es tama " << tama << "su size ECGGGGG " << su.sizeSub() << endl;
		 this->subtrmas.push_back(su);
		 su.~SubTramaECG();
	}
	else if (sub.seleccionMetod(codeType(datas,pos))==1) {
		SubTramaImpedancia su1(sub.getstart(),sub.getvodi(),sub.getsize());
		...
		SubtRamTemp su2(sub.getstart(),sub.getvodi(),sub.getsize());
		...
		SubTramSpo2 su3(sub.getstart(),sub.getvodi(),sub.getsize());
		...
		SubTramaArt_AP su4(sub.getstart(),sub.getvodi(),sub.getsize());
		...
		pos = sub.addData(datas, pos,sub.sizePSubtrama());
		this->tam+= tama + sub.sizeSub();
		this->subtrmas.push_back(sub);
